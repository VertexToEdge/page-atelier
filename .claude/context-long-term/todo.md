...
- [x] LLM 어댑터 구현 - packages/llm/src/index.ts에 generateJSON 함수 구현
  - Provider-agnostic 인터페이스 설계 (LLMAdapter 추상 클래스)
  - Gemini/OpenAI 어댑터 구현 (241줄)
  - 재시도 로직 및 에러 핸들링 (exponential backoff)

- [x] 설정노트 생성 모듈 구현 - packages/core/src/settingBuilder.ts 작성
  - Characters, Relations, World Rules, Timeline 추출 (174줄)
  - 프롬프트 템플릿 구현
  - JSON 후처리 로직 (enhanceSettingNote)

- [x] 일관성 체크 모듈 구현 - packages/core/src/checker.ts 작성
  - 개연성(continuity) 점검 (237줄)
  - 캐릭터(character) 일관성 검사
  - 세계관 규칙(world_rules) 위반 감지
  - 가중합 점수 계산 로직

- [x] 페르소나 평가 모듈 구현 - packages/core/src/personas.ts에 3종 페르소나 구현
  - 설정 과몰입형 페르소나 (273줄)
  - 로판 서브주총러 페르소나
  - 정통무협팬 페르소나
  - 각 페르소나별 평가 프롬프트 및 메트릭

- [x] 종합 리포트 생성 모듈 구현 - packages/core/src/aggregate.ts 작성
  - 가중합 점수 계산 (continuity 0.4, character 0.35, world_rules 0.25)
  - Verdict 결정 로직 (PASS/REVISE/BLOCK) (337줄)
  - 액션 아이템 생성 및 우선순위 관리

### 4단계: API 구현 (15분) ✅ 완료
- [x] API 엔드포인트 구현 - POST /api/analyze 라우트 작성
  - apps/web/app/api/analyze/route.ts 구현 (219줄)
  - 전체 분석 파이프라인 오케스트레이션
  - 에러 핸들링 및 응답 포맷팅
  - GET 메서드로 API 문서 제공

- [x] 프롬프트 템플릿 작성 - 설정노트, 점검, 페르소나, 종합 리포트용
  - packages/core/src/prompts/index.ts 템플릿 관리 (198줄)
  - Temperature 0.3 이하 설정
  - 시스템 프롬프트 및 분석 프롬프트 구조화

- [x] JSON 스키마 검증 로직 구현 - Zod 활용
  - 입력 검증 미들웨어 (AnalyzeRequestSchema)
  - 출력 스키마 준수 확인 (각 모듈별 Zod 스키마)
